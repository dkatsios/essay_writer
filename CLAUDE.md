...
For Render free-plan deployments, keep the combined-container worker count at `1` unless you move workers into a separate service or upgrade memory. The checked-in `render.yaml` sets `ESSAY_WORKER_COUNT=1` for that reason and runs `python scripts/db_upgrade_local.py` as a pre-deploy command before the container starts. The combined runtime entrypoint also runs the same legacy-safe DB upgrade logic before launching the web and worker processes so migrations happen in the same runtime environment.

For deployment debugging, set `ESSAY_WEB_ONLY=true` (or `ESSAY_WRITER_WEB_ONLY=true`) to start only the web process in the combined container. `src.start_web_and_workers` also emits early `[combined-startup] ...` log lines around migration, web spawn, port binding, and worker startup decisions.
