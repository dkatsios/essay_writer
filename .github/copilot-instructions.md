...
- **Staged source filtering** — Phase 1: filter out sources with no useful abstract. Phase 2: batch-triage title+abstract candidates via `source_triage.j2` (batches of `search.triage_batch_size`, default 30), discarding only clearly irrelevant papers. Phase 3: fetch PDF content only for triaged API sources (non-PDF URLs are skipped — analysis showed 89% are garbage HTML). Phase 4: batch-score triaged candidates via `source_scoring.j2` (batches of `search.batch_score_size`, default 50). Phase 5: select top T. Phase 6: full LLM extraction (`source_reading.j2`) on selected sources only. `SourceNote` files are produced only for selected sources.
