...
- **Style control split** — keep writer guidance compact and positive in `style_common.j2` (argument-led prose, concrete wording, paragraph progression). Put explicit cleanup of templated academic tells (essay-about-the-essay scaffolding, over-signposting, paraphrastic repetition) in `style_review.j2` so the reviewer removes them without bloating the writer prompt.
