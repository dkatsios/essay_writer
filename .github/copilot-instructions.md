...
- **Parallel source reading** — `ThreadPoolExecutor(max_workers=3)` reads up to `2 × target_sources` registry sources concurrently. `_select_best_sources` then picks the best `target_sources` for the essay.
