...
- **Shared HTTP transport** — search APIs and URL fetching use a shared `httpx.Client` with centralized retry behavior and connection pooling in `src/tools/_http.py`. PDF downloads use `curl_cffi` with Chrome TLS fingerprint impersonation (`pdf_get()`) to bypass publisher bot-detection; non-PDF requests use `httpx` (`http_get()`).
