...
`src/web.py` is a FastAPI app that wraps the same pipeline used by the CLI. A single HTML page (`src/templates/web/index.html`) provides a form (prompt, file upload, target word count). Jobs run in background threads; validation questions pause the pipeline thread via `threading.Event` and are pushed to the browser via Server-Sent Events (SSE). Results are returned as a ZIP containing the docx, markdown, and sources metadata.
