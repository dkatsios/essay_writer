...
# Temporary debug switch: ESSAY_WEB_ONLY=true starts only the web process
...
**Combined startup diagnostics** — set `ESSAY_WEB_ONLY=true` (or `ESSAY_WRITER_WEB_ONLY=true`) to make the combined Docker entrypoint skip worker startup and run only the web process. `src.start_web_and_workers` emits early `[combined-startup] ...` log lines for migration start/failure, web spawn, port-bind success, and worker spawn or skip decisions.

**Render free plan** — the checked-in `render.yaml` sets `ESSAY_WORKER_COUNT=1` because the combined container can exceed the 512 MiB memory limit if it starts the default 6 workers. It also runs `python scripts/db_upgrade_local.py` as a pre-deploy command before the container starts. `src.start_web_and_workers` runs the same legacy-safe DB upgrade logic again in the runtime container before launching the web and worker processes, then waits for the web port to bind before spawning workers so Render can detect the listening port promptly.
...
- **History inspection surface** — `GET /history` serves the browser history page. `GET /history/jobs` lists persisted runtime summaries, including active jobs immediately after submission plus the current persisted step overlay when available (with optional `limit` and `status` query params). `GET /history/jobs/{job_id}` returns the persisted runtime summary, step metrics, artifact manifest, and the current live status payload when that job is still active. `POST /history/jobs/{job_id}/cancel` cancels a queued or active job and preserves its history as terminal `cancelled`. `POST /history/jobs/{job_id}/remove` cancels the job if needed and then removes the `web_jobs` row, runtime summary, step metrics, artifact metadata, and run storage artifacts.
