...
5. **Read sources** (worker) — staged: (a) title+abstract triage via `source_triage.j2` → `SourceTriageBatch`, (b) fetch PDF content only for triaged API sources (non-PDF URLs skipped), (c) batch-score triaged candidates via `source_scoring.j2` → `SourceScoreBatch`, (d) select top T with a stricter minimum relevance threshold, (e) full extraction on selected sources via `source_reading.j2` → `SourceNote`
...
|------|-----------------|--------|-----------|-----------|----------|
| **worker** | `gemini-2.5-flash` | `gpt-5.4-nano` | `claude-haiku-4-5` | `intake.j2`, `validate.j2`, `plan.j2`, `source_triage.j2`, `source_scoring.j2`, `source_reading.j2`, `source_assignment.j2` | Structured data extraction (brief, plan, source triage, source scoring, notes, source assignment) |
...
12 templates in `src/templates/`, one per pipeline task. Each template contains a `<!-- SPLIT -->` marker that separates **system instructions** (role identity, behavioral rules, style guidelines) from **user content** (variable data, task description). `render_prompt()` returns a `PromptPair(system, user)` namedtuple; the pipeline helpers route these into separate `system` and `user` messages in the API call. Templates without a marker produce `system=None` (user-only message).
...
| `source_triage.j2` | `essay_topic`, `thesis`, `sections` (list of {title, key_points}), `sources` (list of {source_id, title, abstract}) | `SourceTriageBatch` JSON |
...
- **Config-backed search controls** — `search.fetch_per_api` sets the per-API-per-query fetch limit (default 20), independent of how many sources the essay ultimately uses. `search.triage_batch_size` controls the cheap title+abstract triage batch size (default 30). `search.batch_score_size` controls the heavier 1–5 scoring batch size (default 50). `search.min_relevance_score` controls the minimum final selection score (default 3).
- **Recovery search controls** — `search.recovery_overfetch_multiplier`, `search.recovery_fetch_per_api_multiplier`, and `search.recovery_prefer_fulltext` control the automatic recovery rerun. Recovery can trigger at two points: (1) after triage, if the triaged pool is below `target_sources`, and (2) after scoring, if selected sources are below target and no triage recovery already ran. Only one research rerun happens per pipeline run. Already-triaged source IDs (accepted and rejected) plus DOI/title dedup sets prevent re-processing.
...
- **Staged source filtering** — Phase 1: filter out sources with no useful abstract. Phase 2: batch-triage title+abstract candidates via `source_triage.j2` (batches of `search.triage_batch_size`, default 30), discarding off-topic and tangentially related papers. Phase 2b (conditional): if triaged pool < `target_sources`, recovery research rerun + triage new candidates (skipping already-triaged IDs and DOI/title dupes). Phase 3: fetch PDF content only for triaged API sources (non-PDF URLs are skipped — analysis showed 89% are garbage HTML). Phase 4: batch-score triaged candidates via `source_scoring.j2` (batches of `search.batch_score_size`, default 50). Phase 5: select top T. Phase 5b (conditional): if selected < target and no triage recovery ran, scoring recovery rerun + triage/score new candidates. Phase 6: full LLM extraction (`source_reading.j2`) on selected sources only. `SourceNote` files are produced only for selected sources.
- **`_select_top_sources`** — ranks batch-scored candidates and returns top `target_sources` IDs. Sources with `relevance_score < search.min_relevance_score` are filtered out (default 3). Sort keys (all descending, `reverse=True`): **`user_provided`** → **batch relevance_score** → has authors → **`citation_count`** → **has_fulltext**. If the usable pool is smaller than `target_sources`, the selected set is smaller — not padded.
