...
class TestProxySession:
    """Tests for _ProxySession URL rewriting and auth detection."""
    ...
    def test_hostname_rewriting(self):
        from src.tools._http import _ProxySession

        ps = _ProxySession(proxy_prefix="https://login.proxy.eap.gr/login?url=")
    ...
    def test_url_prefix_mode_fallback(self):
        ...
        ps = _ProxySession(proxy_prefix="https://proxy.uoa.gr/login?url=")
...
class TestParallelPdfFetch:
    ...
    def test_is_pdf_content(self):
        from src.tools._http import _is_pdf_content

        assert _is_pdf_content(b"%PDF-1.4 ...") is True
        assert _is_pdf_content(b"<html>not a pdf</html>") is False
        assert _is_pdf_content(b"") is False

    def test_pick_best_prefers_pdf_over_non_pdf(self):
        from src.tools._http import PdfResponse, _pick_best_pdf
        ...
        assert _pick_best_pdf(html_resp, pdf_resp, "http://x") is pdf_resp
        # direct PDF wins when proxy returns HTML
        assert _pick_best_pdf(pdf_resp, html_resp, "http://x") is pdf_resp
    ...
    def test_pick_best_prefers_direct_when_both_pdf(self):
        ...
        assert _pick_best_pdf(direct, proxy, "http://x") is direct
    ...
    def test_pick_best_prefers_2xx_over_error(self):
        ...
        assert _pick_best_pdf(err, ok, "http://x") is ok
        assert _pick_best_pdf(ok, err, "http://x") is ok

    def test_pick_best_handles_none(self):
        from src.tools._http import PdfResponse, _pick_best_pdf

        resp = PdfResponse(200, b"%PDF-1.4", {})
        assert _pick_best_pdf(resp, None, "http://x") is resp
        assert _pick_best_pdf(None, resp, "http://x") is resp
